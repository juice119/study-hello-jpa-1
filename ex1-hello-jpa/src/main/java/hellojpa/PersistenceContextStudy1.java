...
public class PersistenceContextStudy1 {
    ...
    static void main(EntityManagerFactory emf) {
        ...
        try {
            ...
            member.setId(id);
            member.setUserName("hello Spring is new");
            System.out.println("beforeMember Name: " + member.getUserName());

            // 2. 영속성 상태 - Managed
            System.out.println("영속 상태 -----");
            em.persist(member);
            member.setUserName("hello Spring is Managed");

            /**
             * SELECT 쿼리가 두번 발생하지 않는 이유 1차 캐시에 저장된 member 가져오기 떄문
             */
            Member updatedMember = em.find(Member.class, id);
            System.out.println("updatedMember Name: " + updatedMember.getUserName());
